...
public class simpleExecutor implements Executor {
    ...
    public void update(Configuration configuration, MappedStatement mappedStatement, Object... params) throws Exception {
        ...
        // 2. 获取sql语句 : update xhsdb.user set username=#{username} where id=#{id} and username=#{username}
        //转换sql语句： update xhsdb.user set username=? where id=#{id} and username=#{username} where id = ? and username = ? ，转换的过程中，还需要对#{}里面的值进行解析存储
        String sql = mappedStatement.getSql();
        BoundSql boundSql = getBoundSql(sql);
